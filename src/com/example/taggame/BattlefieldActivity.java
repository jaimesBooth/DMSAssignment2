...
public class BattlefieldActivity extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		actionLable = battlePlayer.getPlayerName()
	...
	private String interpret(String s) {
		...
		if (type.equalsIgnoreCase("update")) {
			...
			switch (result) {
			case "hit":
				translation = attackerID + "(" + attackerHP + ")"
						+ " attacked " + enemyID + "(" + enemyHP + ")";
				break;
			case "counter":
				translation = enemyID + "(" + enemyHP + ")"
						+ " counterattacked " + attackerID + "(" + attackerHP
						+ ")";
				break;
			case "missed":
				translation = enemyID + "(" + enemyHP + ")"
						+ " dodged the attack from " + attackerID + "("
						+ attackerHP + ")";
				break;
			default:
	...
	protected void onActivityResult(int reqCode, int resCode, Intent i) {
		if (i != null) {
			...
			if (reqCode == ACTION_RESPONSE && resCode == RESULT_OK) {
				...
				Toast.makeText(getApplicationContext(),
						bun.getString("resultMessage"), Toast.LENGTH_SHORT)
						.show();
			}
		}
	}
